    ...
    <div className="space-y-8">
      <section className="rounded-xl border border-border bg-elevated/60 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Send real-time Pay On Arc events to your backend.</p>
          </div>
        ...
        {webhooks.length === 0 ? (
          <div className="mt-5 rounded-xl border border-dashed border-white/10 bg-slate-950/40 p-8 text-center">
            <h3 className="text-lg font-semibold text-white">No webhooks yet</h3>
          ...
          <div className="mt-5 overflow-hidden rounded-xl border border-white/10">
            <div className="hidden grid-cols-[1fr_1.4fr_0.7fr_1fr_1.4fr] gap-4 border-b border-white/10 bg-white/[0.03] px-4 py-3 text-xs uppercase tracking-[0.12em] text-slate-500 xl:grid">
              <span>Name</span>
              ...
              {webhooks.map((webhook) => (
                <div key={webhook.id} className="grid gap-4 px-4 py-4 xl:grid-cols-[1fr_1.4fr_0.7fr_1fr_1.4fr] xl:items-center">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">{webhook.secret}</p>
                  </div>
                  <button
                    className="min-w-0 truncate rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2 text-left font-mono text-xs text-slate-300 transition hover:border-violet-400/30"
                  ...
                  <p className="text-sm text-slate-400">{lastDelivery(webhook)}</p>
      ...
      <section className="rounded-xl border border-border bg-elevated/60 p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Inspect webhook payloads, headers, responses, and retries.</p>
          </div>
        ...
        {filteredDeliveries.length === 0 ? (
          <div className="mt-5 rounded-xl border border-dashed border-white/10 bg-slate-950/40 p-8 text-center">
            <h3 className="text-lg font-semibold text-white">No deliveries found</h3>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-400">
          ...
          <div className="mt-5 overflow-hidden rounded-xl border border-white/10">
            <div className="hidden grid-cols-[1fr_1fr_0.7fr_0.7fr_0.7fr_auto] gap-4 border-b border-white/10 bg-white/[0.03] px-4 py-3 text-xs uppercase tracking-[0.12em] text-slate-500 xl:grid">
              <span>Event</span>
              <span>Endpoint</span>
              <span>Status</span>
              <span>HTTP code</span>
            ...
            <div className="divide-y divide-white/10">
              {filteredDeliveries.map((delivery) => (
                <div key={delivery.id} className="grid gap-4 px-4 py-4 xl:grid-cols-[1fr_1fr_0.7fr_0.7fr_0.7fr_auto] xl:items-center">
                  <div>
                    <p className="font-medium text-white">{delivery.eventType}</p>
                    <p className="mt-1 font-mono text-xs text-slate-500">{delivery.eventId}</p>
                  </div>
                  <p className="truncate text-sm text-slate-400">{delivery.webhookName}</p>
                  <span className={`w-fit rounded-full border px-2.5 py-1 text-xs capitalize ${statusClass(delivery.status)}`}>
                    {delivery.status}
                  </span>
                  <p className="text-sm text-slate-400">{delivery.responseStatus ?? "—"}</p>
                  <p className="text-sm text-slate-400">{delivery.attempt}</p>
                  <div className="flex flex-wrap gap-2 xl:justify-end">
                    ...
                    <p className="basis-full text-xs text-slate-500 xl:basis-auto">{formatDate(delivery.createdAt)}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {createOpen ? (
        <div className="fixed inset-0 z-[100] flex min-h-dvh items-center justify-center overflow-y-auto bg-black/80 p-4 py-8 backdrop-blur-md sm:p-6">
          <div className="relative my-auto w-full max-w-xl rounded-2xl border border-white/10 bg-[#0B0F19] p-5 shadow-premium ring-1 ring-white/5 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-white">{oneTimeSecret ? "Webhook secret" : "Create webhook"}</h3>
            ...
            {oneTimeSecret ? (
              ...
              <form onSubmit={createWebhook} className="mt-5 space-y-4">
                ...
                <label className="block">
                  <span className="text-sm font-medium text-slate-300">Endpoint URL</span>
                  <input
                    className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950/60 px-3 py-2.5 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-violet-400/60 focus:ring-2 focus:ring-violet-500/20"
                    disabled={isBusy}
                ...
                {error ? <div className="rounded-lg border border-rose-400/20 bg-rose-400/10 p-3 text-sm text-rose-100">{error}</div> : null}
      ...
      {selectedDelivery ? (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/70 backdrop-blur-sm">
          <aside className="h-full w-full max-w-2xl overflow-y-auto border-l border-white/10 bg-[#0B0F19] p-5 shadow-premium">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-white">Delivery details</h3>
                <p className="mt-1 text-sm text-slate-400">{selectedDelivery.eventType}</p>
              </div>
              <button
      ...
      {selectedWebhook ? (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/70 backdrop-blur-sm">
          <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-[#0B0F19] p-5 shadow-premium">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-white">Webhook endpoint</h3>
                <p className="mt-1 text-sm text-slate-400">{selectedWebhook.name}</p>
              </div>
              <button
...
function DetailBlock({ title, value }: { title: string; value: unknown }) {
  ...
  return (
    <div>
      ...
      <pre className="mt-2 max-h-96 overflow-auto rounded-xl border border-white/10 bg-slate-950/70 p-4 text-xs leading-5 text-slate-300">
        {content}
      </pre>
    </div>
  );
