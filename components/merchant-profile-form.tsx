...
const fields: Array<{
  ...
  { name: "merchantName", label: "Merchant Name", placeholder: "Ben's Store", helper: "Shown on checkout pages and your merchant profile." },
  { name: "merchantSlug", label: "Merchant Slug", placeholder: "ben-store", helper: "Lowercase letters, numbers, and hyphens only." },
  { name: "merchantEmail", label: "Merchant Email", placeholder: "merchant@example.com", type: "email" },
  { name: "supportEmail", label: "Support Email", placeholder: "support@example.com", type: "email" },
  { name: "websiteUrl", label: "Website URL", placeholder: "https://example.com", type: "url" },
  { name: "logoUrl", label: "Logo URL", placeholder: "https://example.com/logo.png", type: "url" },
];
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {fields.map((field) => (
          <label key={field.name} className="block">
            <span className="text-sm font-medium text-starlight">{field.label}</span>
            <input
              aria-invalid={Boolean(errors[field.name])}
              aria-describedby={`${field.name}-hint`}
              className="mt-2 w-full rounded-lg border border-border bg-[#0B0F19]/70 px-3 py-2.5 text-sm text-starlight outline-none transition placeholder:text-silver/50 focus:border-primary focus:ring-2 focus:ring-primary/20"
              disabled={disabled}
              name={field.name}
            ...
            />
            <span id={`${field.name}-hint`} className="mt-1 block text-xs text-silver">
