...
export function PaymentActions({
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.message ?? payload?.error ?? "Transaction was submitted, but could not be recorded.");
      }

      setTransactionId(payload?.transaction?.id ?? null);
      setMessage("Payment submitted. Transaction status is pending.");
    } catch (payError) {
