...
import { getDashboardStats, getTopPaymentLinks } from "@/lib/dashboard-analytics";
...
export async function POST(request: Request) {
  ...
  const [paymentLinks, transactions, stats, dashboardStats, topLinks] = await Promise.all([
    ...
    getTopPaymentLinks(auth.user.id, 3),
  ]);
  ...
  const compactSummary = {
    totals: {
      ...
      averagePaymentSize: dashboardStats.averagePaymentSize.toFixed(6),
      revenueLast7Days: dashboardStats.revenue7d.toFixed(6),
      revenueLast30Days: dashboardStats.revenue30d.toFixed(6),
      revenue7DayGrowthPercent: dashboardStats.growth.revenue7d.toFixed(2),
      revenue30DayGrowthPercent: dashboardStats.growth.revenue30d.toFixed(2),
    },
    topPaymentLinks: topLinks.map((link) => ({
      title: link.title,
      revenue: link.revenue.toFixed(6),
      paymentsCount: link.paymentsCount,
      status: link.status,
    })),
  ...
  const prompt = [
    ...
    "Return five short sections: Revenue summary, Best performing payment link, Revenue trends, Conversion patterns, Suggested next action.",
