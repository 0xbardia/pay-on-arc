...
import { arcChainId, arcUsdcDecimals, getArcUsdcAddress } from "@/lib/arc-config";
import { isPaymentLinkExpired } from "@/lib/payment-links";
import { verifyUsdcPaymentTx } from "@/lib/payment-verification";
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    const existingHash = await prisma.transaction.findUnique({
      where: { txHash },
    });
    ...
    if (!paymentLink) {
      return paymentError("PAYMENT_LINK_NOT_FOUND", "Payment link not found.", 404);
    }
    ...
    if (paymentLink.status !== PaymentLinkStatus.ACTIVE || isPaymentLinkExpired(paymentLink.expiresAt)) {
      return paymentError("PAYMENT_LINK_UNAVAILABLE", "Payment link is not available.", 400);
    }
    ...
    if (!merchantWallet || !isAddress(merchantWallet)) {
      return paymentError("MERCHANT_WALLET_MISSING", "Merchant wallet is not configured.", 400);
    }

    const verification = await verifyUsdcPaymentTx({
      txHash,
      expectedRecipient: merchantWallet,
      expectedAmount: paymentLink.amount.toFixed(6),
    ...
    if (!verification.ok) {
      ...
      return paymentError(
        ...
        verification.message ?? "Payment transaction could not be verified.",
        verification.reason === "RECEIPT_NOT_FOUND" || verification.reason === "RPC_ERROR" ? 503 : 400,
      );
    }

    const transaction = await prisma.$transaction(async (tx) => {
      const duplicateHash = await tx.transaction.findUnique({
        where: { txHash },
      });
      ...
      const transaction = await tx.transaction.create({
        data: {
          ...
          metadata: {
            ...
            verifiedAmount: verification.normalizedAmount,
            verifiedBlockNumber: verification.blockNumber,
            verifiedToken: verification.token,
          },
        },
      });
    ...
    void emitWebhookEvent({
      ...
      if (process.env.NODE_ENV !== "production") {
        console.error("[webhooks] payment.confirmed emit failed", error);
      }
    });
