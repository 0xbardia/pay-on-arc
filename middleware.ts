...
export function middleware(request: NextRequest) {
  ...
  if (pathname === "/app" || pathname.startsWith("/app/")) {
    const sessionCookie = request.cookies.get("arcpay_session")?.value;
    if (!sessionCookie) {
      return applySecurityHeaders(NextResponse.redirect(new URL("/", request.url)));
    }
    // Set a cookie to indicate session was validated (client-readable)
    const response = NextResponse.next();
    response.cookies.set("x-session-validated", "true", {
      httpOnly: false,
      sameSite: "lax",
