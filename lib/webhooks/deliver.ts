...
function scheduleRetry(deliveryId: string, delayMs: number) {
...
export async function emitWebhookEvent({
  ...
  for (const delivery of deliveries) {
    void processWebhookDelivery(delivery.id);
  }
}
...
export async function processDueWebhookDeliveries() {
  ...
  await Promise.all(deliveries.map((delivery) => processWebhookDelivery(delivery.id)));
}
...
async function markDeliveryFailure({
  ...
  if (!exhausted && nextDelay) {
    scheduleRetry(deliveryId, nextDelay);
  }
}
